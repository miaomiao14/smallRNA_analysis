...
foreach my $strand (keys %readMap)
{
	...
	foreach my $chr (keys %{$readMap{$strand}})
	{
		...
		print "\n\nNow trying with fork:\n\n";
		my $starttime = Benchmark->new;
		&disProcessFork( \%{$readMap{$strand}{$chr}},$strand,$chr,\%disScoreChrFork);
...
while (my ($key, $value) = each %disScoreChrFork) {
	while (my ($key1, $value1) = each %{$value}) {
		while (my ($key2, $value2) = each %{$value1}) {
			$disScoreFork{$key2}+=$value2;
		}
	}
}
...
foreach my $dis (sort { $a <=> $b } keys %disScoreFork)
{
	print OUT "$dis\t$disScoreFork{$dis}\n";
}
close(OUT);


# open OUT, ">$OUTDIR/$file1.wofork.5-5.distance.distribution" or die "cannot write to $OUTDIR: $!\n";
# foreach my $strand (keys %disScoreChr)
# {
...
open OUT, ">$OUTDIR/$file1.wofork.5-5.distance.distribution.summary" or die "cannot write to $OUTDIR: $!\n";
