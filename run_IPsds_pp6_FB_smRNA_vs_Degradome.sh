...
if [ ! -f ${OUT}/.status.${STEP}.pp6sds.SRA_vs_SRA ] 
then
for t in ${GT[@]}
do
	for o in ${OX[@]}
...
[ $? == 0 ] && \
touch ${OUT}/.status.${STEP}.pp6sds.SRA_vs_SRA
STEP=$((STEP+1))
...
if [ ! -f ${OUT}/.status.${STEP}.pp6sds_FB_SRA_vs_SRA ] 
then
	for t in ${GT[@]}
	do
		for o in ${OX[@]}
...
[ $? == 0 ] && \
touch ${OUT}/.status.${STEP}.pp6sds_FB_SRA_vs_SRA
STEP=$((STEP+1))	
