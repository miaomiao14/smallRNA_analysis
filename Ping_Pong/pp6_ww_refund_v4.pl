...
#########################non ping-pong species,do nothing#####################
##############################################################################
...
for ($i=0; $i<$ARGV[2]; $i++) {
   ...
   open PPlocal, ">$file1_$file2.local.pp";
   open PPZ, ">$file1_$file2.zscore.out";
   open PPZlocal, ">$file1_$file2.lcoalzscore.out";
...
####################################################################################################################################################################################
  ...
  foreach $species (keys %pp_10)
  {
      ############################################################################
      #########################For test, for ping-pong species, times 10############
      ##############################################################################
      if($pos{$pp_10{$species}})
      {
         $pos{$pp_10{$species}}=$pos{$pp_10{$species}} * 10;  ##because both + and - strands will be scanned
      
      foreach $n (keys %{$pp{$species}})
      {  
         foreach $species_pair (keys %{$pp{$species}{$n}}) ##M1,M2,...M20
         {
            $pp{$species}{$n}{$species_pair}=$pp{$species}{$n}{$species_pair} * 10;  
         }
      }
      }
  }
...
####################################################################################################################################################################################     
   ...
   foreach ($n=1;$n<=32;$n++)
   {
      ...
      $m=&mean(values %score);
      if($X>1)  ##if the signal itself is very low
      {
