...
for ($n=-50;$n<=50;$n++) {
   $len=length($_[4]);
if ($_[3] eq '+') { $start=$_[1]+$n-($len+1); $str=substr($genome{$_[0]},$start,$len); $str=&revfa($str); $hash1{$file}{$n}{$str}+=$_[5]/$_[6];}
...
foreach ($n=-50;$n<=50;$n++) {
  %NTM=();
  foreach $seq (keys %{$hash1{$file1}{$n}})
  {
   
      if($hash2{$file2}{$seq})
      {
      $s_min{$n}+=&min($hash1{$file1}{$n}{$seq},$hash2{$file2}{$seq});
      $s_max{$n}+=&max($hash1{$file1}{$n}{$seq},$hash2{$file2}{$seq});
      $s_mean{$n}+=&mean($hash1{$file1}{$n}{$seq},$hash2{$file2}{$seq});
      $s_mul{$n}+=$hash1{$file1}{$n}{$seq}*$hash2{$file2}{$seq};
      $score{$n}+=$hash1{$file1}{$n}{$seq}*$hash2{$file2}{$seq};
      print OUT2 "$seq\n" if ($n==10);
      print OUT21 "$seq\n"if ($n==-16);
      }
  }  
