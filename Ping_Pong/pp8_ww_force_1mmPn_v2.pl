...
for ($i=0; $i<$ARGV[2]; $i++)
{
   ...
   if ($total{$file1}<10 || $total{$file2}<10) { print "$file1-$file2\t-10"; print O "$file1-$file2\t-10"; print OUT1 "$file1-$file2\tNA";} 
         ...
         $X=0; $Z=0; %score=(); %species=();%pairs=();
         open OUT1,">$file2.$file1.1.1mm.pp_fraction";
         print OUT1 "pp";
         open O, ">$file2.$file1.1.1mm.zscore.out";
        open OUT, ">$file2.$file1.1.1mm.pp";
        open OUT2, ">$file2.$file1.1.1mm.ppseq";
        foreach ($n=1;$n<=20;$n++)
        {
        # file1 as ref
         ...
         foreach ($m=0;$m<=15;$m++)
         {
            %count_N=();$X=0;$Z=0;$p=$m+1;
            $std=0;$n_of_species=0;$n_of_s=0;$n_of_p=0;$total=0;$percentage=0;
            foreach ($n=1;$n<=20;$n++)
            {         
            ...
            print OUT "$n\t$n_of_species\t$n_of_pairs\t$score{$m}{$n}\t$p\n";
	   ...
	   #$p=$m+1;
           print O "$file2-$file1\t$p\t$n_of_s\t$n_of_p\t$Z\t$percentage\n";
           print "$file2-$file1\t$p\t$n_of_s\t$n_of_p\t$Z\t$percentage\n";
         
           if ($Z!=-10) { print OUT1 "$file2-$file1\t$p\t",$X/$total{$file1}/$total{$file2}*1000000,"\n";} else { print OUT1 "$file2-$file1\t$p\tNA\n";}
           $N1=`match.pl $file2.$file1.1.1mm.ppseq $file2.seq | sumcol+ 2`; chomp($N1);
           
           open OUT3, ">$file2.$file1.1.1mm.$m.pp_reads_percentage";
           print OUT3 "mismatch","$p\t", "$N1\t$total{$file2}\t",$N1/$total{$file2},"\n";
         }  
        ...
        # file2 as ref
         ...
         $X=0; $Z=0; %score=(); %species=();%pairs=();
         open OUT1,">$file1.$file1.2.1mm.pp_fraction";
         print OUT1 "pp";
         open O, ">$file1.$file2.2.1mm.zscore.out";
        open OUT, ">$file1.$file2.2.1mm.pp";
        open OUT2, ">$file1.$file2.2.1mm.ppseq";
      foreach ($n=1;$n<=20;$n++)
      {
         `bowtie $file2.$n -r -a -v 1 -p 8 $file1.seq --suppress 1,4,6,7 | grep + > $file1.$file2.$n.bowtie.out`;
         foreach ($m=0;$m<=15;$m++)
         {
            %NTM=();
        ...
        foreach ($m=0;$m<=15;$m++)
         {
           ...
           if ($std>0 && $count_N{$m}>=5) { $Z=($X-&mean(values %{$score{$m}}))/$std;} else {$Z=-10;}
	   #$p=$m+1;
           print O "$file1-$file2\t$p\t$n_of_s\t$n_of_p\t$Z\t$percentage\n";
           print "$file1-$file2\t$p\t$n_of_s\t$n_of_p\t$Z\t$percentage\n";
         
           if ($Z!=-10) { print OUT1 "$file1-$file2\t$p\t",$X/$total{$file1}/$total{$file2}*1000000,"\n";} else { print OUT1 "$file1-$file2\t$p\tNA\n";}
           $N1=`match.pl $file1.$file2.2.1mm.ppseq $file1.seq | sumcol+ 2`; chomp($N1);
           
           open OUT3, ">$file1.$file2.2.1mm.$m.pp_reads_percentage";
           print OUT3 "mismatch","$p\t", "$N1\t$total{$file1}\t",$N1/$total{$file1},"\n";
