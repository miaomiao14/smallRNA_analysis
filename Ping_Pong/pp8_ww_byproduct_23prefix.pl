...
next if (/data/);
$total{$file}+=$_[5]/$_[6];
$hash2{$file}{substr($_[4],0,23)}+=$_[5]/$_[6];
for ($n=-50;$n<=50;$n++) {
if ($_[3] eq '+') { $start=$_[1]+$n-24; $str=substr($genome{$_[0]},$start,23); $str=&revfa($str); $hash1{$file}{$n}{$str}+=$_[5]/$_[6];}
else {  $start=$_[2]-$n; $str=substr($genome{$_[0]},$start,23); $hash1{$file}{$n}{$str}+=$_[5]/$_[6];}
}
}
if ($total{$file}>10) {
open OUT, ">$file.seq";
