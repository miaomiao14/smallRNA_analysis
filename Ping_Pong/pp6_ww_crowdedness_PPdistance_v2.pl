...
for ($i=0; $i<$ARGV[2]; $i++) {
   ...
   if($namefield[10])
   {$name1=$namefield[0]."_".$namefield[2]."_".$namefield[1]."_".$namefield[10];}
   else
   ...
   if($namefield[10])
   {$name2=$namefield[0]."_".$namefield[2]."_".$namefield[1]."_".$namefield[10];}
   else
         ...
         foreach $p (sort { $a <=> $b } keys %ppchrpos)
         {
            ...
            if($p>$last_pos)
            {
               ...
               for($i=1;$i<10000000;$i++)
               {
                  ...
                  {
                     $k+=$pp_pos_weight{$chr}{$strand}{$current_pos};  ##be careful about this $k; if it is reinitialized or not
                  }
                  else
                  {
   ...
   {
      ## the crowd_weight for weighted crowdedness, use for 
      print CROWD "$crowd\t$crowd{$crowd}\t$crowd_weight{$crowd}\t$total_pp_reads\n";
   }
   #print CROWD "0\t$total_pos\t$total_pp_reads\t$total_pp_reads\n";
   close(CROWD);
   ...
   open OUT, ">$file1_$file2.distance.normreads.min.distribution";
    ...
    close (OUT);
    open OUT, ">$file1_$file2.distance.normreads.product.distribution";
    ...
    open OUT, ">$file1_$file2.distance.normreads.distribution.min.summary";
    ...
    open OUT, ">$file1_$file2.distance.normreads.distribution.product.summary";
