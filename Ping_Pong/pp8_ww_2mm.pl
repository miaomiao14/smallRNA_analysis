        ...
        foreach ($m=1;$m<=15;$m++)
         {
           ...
           `cut -f2 $file1.$file2.ppseq > $file1.$file2.$p.ppseq.temp`;
           $N2=`match.pl $file1.$file2.ppseq.$m.temp $file2.seq | sumcol+ 2`; chomp($N2);
           open OUT3, ">$file1.$file2.$p.pp_reads_percentage";
           print OUT3 "seedmismatch","$p\t", "$N1\t$total{$file1}\t",$N1/$total{$file1},"\t","$N2\t$total{$file2}\t",$N2/$total{$file2},"\n";
