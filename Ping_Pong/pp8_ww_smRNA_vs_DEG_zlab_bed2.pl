...
#08/07/2014
#modify to fit ghpcc
#$fa="/home/ww74w/smallRNApipeline/pipeline_dm/common/fasta/dmel-all-chromosome-r5.5_TAS.fasta";
...
for ($i=0; $i<$ARGV[2]; $i++)
{
	...
	#while(<IN>) 
	{
		...
		{  
			next if (length($seq)>29 || length($seq)<23); #can not use it for DEG
		}
		next if ($line=~/data/);
		$total{$file}+=$reads/$ntm;
		#$hash2{$file}{substr($_[4],0,16)}+=$_[5]/$_[6]; #here is the problem for DEG
		
		if($strand eq '+')
		{
			$seqstart=$bedstart; #bed2 is 0-based
            $hash2{$file}{substr($genome{$chr},$seqstart,16)}+=$reads/$ntm; #hash2 is the guide
		}
		else
		{
			$seqstart=$bedend-16;
     		$seqtemp=substr($genome{$chr},$seqstart,16);
     		$seqtemp=&revfa($seqtemp);
     		$hash2{$file}{$seqtemp}+=$reads/$ntm; #hash2 is the guide
		}
		...
		for ($n=1;$n <=16;$n++)
		{
			if ($strand eq '+')
			{
				$start=$bedstart+1+$n-17;
				$str=substr($genome{$chr},$start,16);
				$str=&revfa($str);
                $hash1{$file}{$n}{$str}+=$reads/$ntm;     #str is the target; hash1
                push @{$hash3{$file}{$n}{$str}},$seq if($n==10); #hash3, key is the target, value is the guide
