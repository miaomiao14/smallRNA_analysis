...
for($i=0;$i<$ARGV[2];$i++)
{
	...
	open TEMP, ">temp.$n.bed";
	foreach $chr (keys %bed)
	{
	    @bedref=@{$bed{$chr}};
	    for($i=0;$i<=$#bedref;$i++)
	    {
	    print TEMP "$chr\t$bed{$chr}[$i][0]\t$bed{$chr}[$i][1]\t$bed{$chr}[$i][2]\t-1\t$bed{$chr}[$i][3]\n";
	    }
	}
	close(TEMP);
	`fastaFromBed -fi /home/hanb/nearline/Mus_musculus/UCSC/mm9/Sequence/WholeGenomeFasta/genome.fa -bed temp.$n.bed -fo temp.$n.fa -name -s`;
	open FA, "temp.$n.fa";
	while($ano=<FA>)
	{
	$str=<FA>;
