...
#allows 1mm at any position in p1-p16
...
for ($i=0; $i<$ARGV[2]; $i++)
{
   ...
   if ($total{$file1}<10 || $total{$file2}<10) { print "$file1-$file2\t-10"; print O "$file1-$file2\t-10"; print OUT1 "$file1-$file2\tNA";} 
        ...
        foreach ($m=0;$m<=15;$m++)
         {
            %count_N=undef;$X=0;$Z=0;$p=$m+1;
            $std=0;
            foreach ($n=1;$n<=20;$n++)
            {         
            $score{$m}{$n}=0 if (!exists $score{$m}{$n});
            print OUT "$n\t$score{$m}{$n}\t$p\n";
	   ...
	   #$p=$m+1;
           print O "$file1-$file2\t$p\t$Z\n";
           print "$file1-$file2\t$p\t$Z\n";
           ...
           `cut -f2 $file1.$file2.ppseq > $file1.$file2.$m.ppseq.temp`;
           $N2=`match.pl $file1.$file2.ppseq.$m.temp $file2.seq | sumcol+ 2`; chomp($N2);
           open OUT3, ">$file1.$file2.$m.pp_reads_percentage";
           print OUT3 "mismatch","$p\t", "$N1\t$total{$file1}\t",$N1/$total{$file1},"\t","$N2\t$total{$file2}\t",$N2/$total{$file2},"\n";
