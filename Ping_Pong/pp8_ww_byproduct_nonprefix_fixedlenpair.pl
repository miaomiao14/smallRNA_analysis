...
if ($total{$file1}<10 || $total{$file2}<10) { print "$file2-$file1\t-10"; print O "$file2-$file1\t-10"; print OUT1 "$file2-$file1\tNA\n";} 
 ...
 $X=0; $Z=0; %score=();$count_N=0; %s=();
open OUT, ">$file2.$file1.min.pp";
open OUT5, ">$file2.$file1.product.pp";
...
foreach ($n=-50;$n<=50;$n++) {
  ...
  foreach $seq (keys %{$hash1{$file1}{$n}})
  {
      ...
      if($hash2{$file2}{$seq})
      {
      $score{$n}+=&min($hash1{$file1}{$n}{$seq},$hash2{$file2}{$seq});
      $s{$n}+=$hash1{$file1}{$n}{$seq}*$hash2{$file2}{$seq};
      print OUT2 "$seq\n" if ($n==10);
      }
  }  
