...
# take the suffix 16nt and merge them
#check if the overlap peaks at 16nt?
## 3'-3' end distance
...
for ($i=0; $i<$ARGV[2]; $i++)
{
   ...
   #next if (length($_[4])>29 || length($_[4])<23);
      next if (/data/);
      $total{$file}+=$_[5]/$_[6];
      $hash2{$file}{substr($_[4],-16)}+=$_[5]/$_[6];  ## suffix 16nt
      for ($n=-50;$n<=50;$n++)
      {
         if ($_[3] eq '+')
         {
         $start=$_[2]-$n;  ##use end($_[2]) instead of start($_[1])
         ...
         {  $start=$_[1]+$n-17; $str=substr($genome{$_[0]},$start,16);  ##use start($_[1])  instead of end($_[2])
            $hash1{$file}{$n}{$str}+=$_[5]/$_[6];
            push @{$hash3{$file}{$n}{$str}},$_[4];
         }
      }
