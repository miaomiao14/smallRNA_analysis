      ...
      while(<IN>)
      {
         ...
         if ($4 eq '+')
         {
            foreach ($n=1;$n<=32;$n++)
            {
               ...
               $pp_10{"$1:$2+"}="$1:$start-" if($n==9);  #record pp9 for decision, key is the N1, value is the M1
         ...
         else
         {
            foreach ($n=1;$n<=32;$n++)
            {
               ...
               $pp{"$1:$3-"}{$n}{"$1:$start+"}+=$_[1]/$_[6]; ##record each species, and their corresponding pairs when 5' end distance is 1,2,...20
                                                            ## the reads number is from N1
               $pp_10{"$1:$3-"}="$1:$start+" if($n==9); #record pp9 for decision
               #$pp_seq{"$1:$start+"}=$_[0] if($n==10); ## to make sure the key are unique, won't be overwrite by some non-10 nt overlap sequence pair
            } #shift n
         }
         #print "hello\n";
         ...
         ###adjusted in v3
            ...
            {  
                  ...
                  foreach $species_pair (keys %{$pp{$species}{$n}}) ##M1,M2,...M20
                  {
                     ...
                     {
                        if($n!=9 && $pos{$pp_10{$species}}) #if n=10, no need to refund
                        {
                        ...
                        if ($pp{$pp_10{$species_pair}}{9}{$species_pair}) #for M1 if N10 exists
                        
                        {
                           if ($n!=9) #if n=10, no need to refund
                           {
                           $temp2=&max(0,$pos{$species_pair}*($pp{$species}{$n}{$species_pair}-$pp{$pp_10{$species_pair}}{9}{$species_pair}) ); #M1N1-M1N10 for n=1
                           }
                           else
                           {
                           ##############################################################################
...
####################################################################################################################################################################################     
   ...
   foreach ($n=1;$n<=32;$n++)
   {
      ...
      $m=&mean(values %score);
      if($p9>1)  ##if the signal itself is very low
      {
         if ($std>0) ##in terms of refund, when signals are very pure, std=0
         {
            $Z=($p9-&mean(values %score))/$std;
         }
         else
         {
            $Z=10000;
