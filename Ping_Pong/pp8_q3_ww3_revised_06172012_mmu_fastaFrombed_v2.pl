...
##
#open IN, "/home/hanb/nearline/Mus_musculus/UCSC/mm9/Sequence/WholeGenomeFasta/genome.fa";
...
for($i=0;$i<$ARGV[2];$i++)
{
	...
	$total{$file}+=$_[5]/$_[6];
        next if (/data/);
		    ...
		    #calculate the end
		    $end=$start+$p;
		    $name=$_[5]/$_[6]; #use the score as name
		    
		    #print TEMP "$_[0]\t$start\t$end\t$name\t-1\t-\n";
		    push @{$bed{$n}{$_[0]}}, [$start,$end,$name,"-"];
		    #use fastaFrombed get the expected sequence with strandedness
		    ...
		    $end=$start+$p;
		    $name=$_[5]/$_[6];
		    #put the chr start end name score strand into a bed file
		    #print TEMP "$_[0]\t$start\t$end\t$name\t-1\t+\n";
		    push @{$bed{$n}{$_[0]}}, [$start,$end,$name,"+"];
		    #use fastaFrombed get the expected sequence with strandedness
		    #`fastaFromBed -fi /home/hanb/nearline/Mus_musculus/UCSC/mm9/Sequence/WholeGenomeFasta/genome.fa -bed temp.bed -fo temp.fa -s`;
		    #tail
		    #$str=`tail -1 temp.fa`;
		    #$str=substr($genome{$_[0]},$start,$p);
                    #$hash1{$file}{$p}{$n}{$str}+=$_[5]/$_[6];
    ...
    foreach $dis (keys %bed)
    {
	open TEMP, ">temp.$dis.bed";
	foreach $chr (keys %{$bed{$dis}})
	{
	    @bedref=@{$bed{$dis}{$chr}};
	    for($i=0;$i<=$#bedref;$i++)
	    {
	    print TEMP "$chr\t$bed{$dis}{$chr}[$i][0]\t$bed{$dis}{$chr}[$i][1]\t$bed{$dis}{$chr}[$i][2]\t-1\t$bed{$dis}{$chr}[$i][3]\n";
	    }
	}
	close(TEMP);
	`fastaFromBed -fi /home/hanb/nearline/Mus_musculus/UCSC/mm9/Sequence/WholeGenomeFasta/genome.fa -bed temp.$dis.bed -fo temp.$dis.fa -name -s`;
	open FA, "temp.$dis.fa";
	while($ano=<FA>)
	{
	$str=<FA>;
...
for ($i=0;$i<$ARGV[2];$i++)
{
	...
	print OUT1 "$file1-$file2\t$p";
		...
		print "\t$Z";
                if ($Z!=-10)
                {
                    print OUT1 "\t", $X/$total{$file1}/$total{$file2}*1000000; #what dose this mean?
                }
                else
                {
                    print OUT1 "\tNA";
