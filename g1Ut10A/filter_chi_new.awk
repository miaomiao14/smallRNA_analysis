...
}
{
	...
	else if ($NF >= p_value_cutoff)  # if the sequence is not uniquely associated, test the chi score/p value and fold change between most enriched and the least depleted
	{
        ...
        most_abundance_reads = 0;
        second_most_abundance_reads = 0;
        most_enriched_col = 0;
		for (j=2; j<NF; j+=2) 
		{
			if ($j > $(j+1))
            ...
            if ($j > most_abundance_reads)
            {
                second_most_abundance_reads = most_abundance_reads;
                most_abundance_reads = $j;
                most_enriched_col = j;
            } 
            else if ($j > second_most_abundance_reads) 
            {
                second_most_abundance_reads = $j;
            }
		}
		if ( (number_of_sample_more_enriched == 1) && (most_abundance_reads/second_most_abundance_reads >= fold_thres) ) # the reason p value is small is because one sample has this sequence enriched and the other two have it depeleted 
		{
			if (most_enriched_col==2)  # Sample1
			{
                printf "%s\t%d\n", $1, $2 > ARGV[1]".Sample1.Enriched";
				printf "%s\t%d\n", $1, $2 > ARGV[1]".Sample1.Enriched_fold";
			}
			else if (most_enriched_col==4) # Sample2
			{       
                printf "%s\t%d\n", $1, $4 > ARGV[1]".Sample2.Enriched";
				printf "%s\t%d\n", $1, $4 > ARGV[1]".Sample2.Enriched_fold";
			}
			else if (most_enriched_col==6) # Sample3
			{
                printf "%s\t%d\n", $1, $6 > ARGV[1]".Sample3.Enriched";
				printf "%s\t%d\n", $1, $6 > ARGV[1]".Sample3.Enriched_fold";
			}
		}
	}
}
