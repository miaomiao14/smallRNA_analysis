...
#06/26/2014
#run hetero Ping-Pong only
#run n=9 only

#05/21/2015
#more generic (incorporating DEG data)
#fasta: take standard input files(use Bo's version)
#no need to record the first nucleotide identify
#report the coordinates of targets from trans pair
...
my %genome=();
#store genome
if($spe eq "fly" or "mouse" or "gfp")
{
	open IN, $fastafile;
	while(my $line=<IN>)
	{
	   #if ($line=~/>(.+) type/) ###outdated
	   if ($line=~/>(.+)/) 
	   {
	      #$chr="chr$1";###outdated
		  $chr="$1";
...
#main, preprocessing
if($indexFlag) #when index files are not built
{
...
sub InputFileProcessing
{
	...
	my $gz = gzopen($inputfile, "rb") or die "Cannot open $inputfile: $gzerrno\n" ;
   	while($gz->gzreadline($line) > 0) ##read each line here
   	{
		chomp $line;
		#split(/\t/);
		my $chr;
        	...
        	if ($strand eq '+') #target strand information
         	{
	            ...
	            if($supStart >= 0)
	            {
	            	...
	            	if ($endPostion <= $chrSize{$chr}) #exclude reads exceed the right boundary
									...
									{	
										...
										{ ###}
											...
											my $corPairReads=$guideQueryCorReads*$targetpiIndexCorReads/($NTMG{$l[2]}*$NTMT{$l[1]});
							       			...
							       			$transPairSuppSpeciesTotal{$diffstr}+=$corPairSpecies if ($n==9);
							       			$transPairSuppReadsTotal{$diffstr}+=$corPairReads if ($n==9);
										}#TrecordSup
									}#Trecord
								}#GrecordSup
							}#Grecord
											...
											my $corPairReads=$guideQueryCorReads*$targetpiIndexCorReads/($NTMG{$l[2]}*$NTMT{$l[1]});
									       	...
									       	{				
												...
												$cisPairReads{$g_0_nt.$t_9_nt}{$n}{$l[2]}+=$corPairReads;		       			      							
				      							##print out paired piRNA species      							
				      							print PPSEQPAIR "cis\t$piQuery\t$Grecord\t$piGuideSuppSeq\t$guideQueryCorReadsNorm\t$piTargetIndex\t$Trecord\t$piTargetIndexSuppSeq\t$targetpiIndexCorReadsNorm\t$diffstr\n" if ($n==9);
