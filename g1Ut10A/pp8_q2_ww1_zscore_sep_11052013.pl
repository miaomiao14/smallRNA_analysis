...
##pp6
#the output of pp6 is the number of pairs(AT,TA,CG,GC) in cis and a hash table of those could not find pairs;
#and two hashes correspond to two different dataset from specified Piwi-argonautes
...
{
	# make bowtie index
	for ($i=0; $i<$ARGV[2]; $i++) #instead of scanning the input files, should scanning the hash tables here; the hash table needs the 
	{
...
# file1 as ref
	...
	$bowtieOut="$OUTDIR/$file1.$file2.$n.bowtie.out";
 	...
 	#6. read qualities
 	#7. -M
 	#8. comma-separted list of mismatch descriptors
   %NTM=();
   ...
   #bowtieindex: name of the reference sequence, which is the target
