		    ...
		    my $nnGcorTcor=$nGcor*$nTcor; #total species pairs for this prefix
	      	...
	      	if ($l[3]=~/(\d+):(\w)>(\w)/)
	      	{
		       ...
		       {	
			       ...
			       $transPairSpecies{$g_0_nt.$t_9_nt}{$n}{$l[2]}+=$nnGcorTcor/($NTMG{$l[2]}); #use $l[2] as key, indicate how many guide prefix species
			       #trans PingPong pair in reads
			       $transPairReads{$g_0_nt.$t_9_nt}{$n}{$l[2]}+=$gttotal/($NTMG{$l[2]});
			       if ($n==9)
			       {
			       	foreach my $piQuery (keys %{$guidepfsplit{$guideStrandFile}{$l[2]}} )
				    {
