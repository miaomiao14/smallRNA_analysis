...
declare -a OX=("unox")
declare -a UNIQ=("uniq")
OUTDIR=/home/wangw1/isilon_temp/ipsmRNA/transposon_piRNA/UA_VA_cistrans_allpiRNAs_exclude_Piwi_baseFraction_prefix20
[ ! -d ${OUTDIR} ] && mkdir $OUTDIR
#touch ${OUT}/.status.${STEP}.all_piRNA_exclude_Piwi.UA_VA
if [ ! -f ${OUT}/.status.${STEP}.all_piRNA_exclude_Piwi.UA_VA_cistrans_baseFraction_prefix20 ]
then 
	for t in ${GT[@]}
	do
		for o in ${OX[@]}
...
[ $? == 0 ] && \
touch ${OUT}/.status.${STEP}.all_piRNA_exclude_Piwi.UA_VA_cistrans_baseFraction_prefix20
STEP=$((STEP+1))
...
OUTDIR=/home/wangw1/isilon_temp/ipsmRNA/transposon_piRNA/UA_VA_cis_trans_baseFraction_prefix20
...
if [ ! -f ${OUT}/.status.${STEP}.transposon_piRNA.UA_VA_cis_trans_baseFraction_prefix20 ] 
then
for t in ${GT[@]}
do
	for o in ${OX[@]}
	do
		...
		do
			...
			[ ! -d ${jOUT} ] && mkdir -p ${jOUT} && \
			/home/wangw1/bin/submitsge 8 ${jobname} $OUTDIR "$script -i ${A} -j ${B} -n 2 -s fly -o ${jOUT} -d ${indexFlag} -f normbed -a ${fa}>${jOUT}/${t}${s}_${o}_Ago3S_AubAS.pp8.q2.UA_VA_cis_trans_baseFraction_prefix20.log"
#			
...
[ $? == 0 ] && \
touch ${OUT}/.status.${STEP}.transposon_piRNA.UA_VA_cis_trans_baseFraction_prefix20
STEP=$((STEP+1))
...
#use xkxh.norm.bed instead of transposon mappers
OUTDIR=/home/wangw1/isilon_temp/ipsmRNA/transposon_piRNA/UA_VA_cis_trans_allpiRNAs_baseFraction_prefix20
#touch ${OUT}/.status.${STEP}.all_piRNA.UA_VA_cis_trans
if [ ! -f ${OUT}/.status.${STEP}.all_piRNA.UA_VA_cis_trans_baseFraction_prefix20 ]
then 
	for t in ${GT[@]}
	do
		for o in ${OX[@]}
