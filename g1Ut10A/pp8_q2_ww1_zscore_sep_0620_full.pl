...
#06/16/2014
#separate cis(pp6,v4) and ambiguous
	      	...
	      	if ($l[3] eq "")
	      	{
			   ...
			   #how many of species start with U?
				...
				foreach my $piQuery (keys %{$guidepfsplit{$guideStrandFile}{$l[2]}} )
			    {
					...
					foreach my $piGuideSpe (keys %{$targetpfsplit{$targetStrandFile}{$n}{$l[1]}} ) #indexed piRNA prefix; iterate each piRNA
					{	
						...
						map {$targetpiGuideReads+=$_} values %{$targetpfsplit{$targetStrandFile}{$n}{$l[1]}{$piGuideSpe}}; # the total reads for this piRNA
						...
						foreach my $record (keys %{$guidepfsplit{$guideStrandFile}{$l[2]}{$piQuery}}) #each mapping locus for guide piRNA
						{
				       		...
				       		my $targetpiGuideCorReads=$targetpfsplit{$targetStrandFile}{$n}{$l[1]}{$piGuideSpe}{"$chr,$tfiveend,$tstrand"} ;
							...
							# for each mapping locus for this guide piRNA, if there is a cis pair from target piRNA coordinate pool	
					       	if($targetpfsplit{$targetStrandFile}{$n}{$l[1]}{$piGuideSpe}{"$chr,$tfiveend,$tstrand"}) #here it checks all coordinates associated with $piGuideSpe
					       	{
								...
								$cisPairReads{$g_0_nt.$t_9_nt}{$n}{$l[2]}+=$guideQueryCorReads*$targetpiGuideCorReads/$NTM{$l[2]};		       			
      							...
      							print PPSEQPAIR "cis\t$piQuery\t$guideQueryCorReadsNorm\t$piGuideSpe\t$targetpiGuideCorReads\n" if ($n==9);
      							#ambiguous pairs if there are other mapping loci
      							my $nGpiCor=scalar (keys %{$guidepfsplit{$guideStrandFile}{$l[2]}{$piQuery}});
      							if( $nGpiCor>1)
      							{
      								$ambiguousPairSpecies{$g_0_nt.$t_9_nt}{$n}{$l[2]}+=1/$NTM{$l[2]};
      								...
      								print PPSEQPAIR "ambiguous\t$piQuery\t$ambiguousGuideQueryCorReadsNorm\t$piGuideSpe\t$ambiguousTargetpiGuideCorReads\n" if ($n==9);
      							}
