...
[ ! -d ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore ] && mkdir -p ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore
#step 1
for g in "${GROUPGT[@]}"
do
	[ ! -d ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore/${g} ] && mkdir -p ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore/${g}
	OUTDIR=${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore/${g}
	LOG=${OUTDIR}/${g}.log
	smmapper2=${smRNAINDIR}/Phil.SRA.${g}.ox.ovary.inserts/Phil.SRA.${g}.ox.ovary.inserts.xkxh.transposon.mapper2.gz
	demapper2=${degraINDIR}/Phil.DEG.${g}.ovary/bedIntersectWW/Phil.DEG.${g}.ovary.x_rRNA.dm3.sorted.f0x40.noS.5p.all.bed.ntm.collapse.xkxh.FLY_TRANSPOSON_ALL.nta.mapper2.gz
	[ ! -f $demapper2 ] && \
	...
	#total Ping-Pong
	[ ! -s ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore/${g}.total.pp6.out ] && submitsge 8 $g "$script ${smmapper2%.gz}.23-29.gz ${demapper2} 2 ${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore >${smRNAINDIR}/pp6_FB_smRNA_vs_degradome_total_with_ppscore/${g}.total.pp6.out" 
