...
parafly_file=${RANDOM}${RANDOM}${RANDOM}.para
for t in ${TARGETS[@]}
do \
...
# running ParaFly
ParaFly -c $parafly_file -CPU $CPU -failed_cmds bedtools_intersecting.paraFly_failed_commands
rm -rf $parafly_file ${parafly_file}.completed

parafly_file=${RANDOM}${RANDOM}${RANDOM}.para
for t in ${TARGETS[@]}
do \
	echo "awk 'BEGIN{OFS=\"\\t\"} {if (ARGIND==1) {ct[\$4]=0; ct_pos[\$4]=0; ct_neg[\$4]=0;} if (ARGIND==2) {ct[\$14]++; if (\$10==\$16){ct_pos[\$14]++} else {ct_neg[\$14]++}}}END{for (i in ct){ print i,ct[i],ct_pos[i],ct_neg[i] }}' ${!t} ${UNIQUE_BED%bam}${t}.bedpe  > ${UNIQUE_BED%bam}${t}.bedpe.ct " >> $parafly_file && \
	echo "awk 'BEGIN{OFS=\"\\t\"} {if (ARGIND==1) {ct[\$4]=0; ct_pos[\$4]=0; ct_neg[\$4]=0;} if (ARGIND==2) {ct[\$14]++; if (\$10==\$16){ct_pos[\$14]++} else {ct_neg[\$14]++}}}END{for (i in ct){ print i,ct[i],ct_pos[i],ct_neg[i] }}' ${!t} ${MULTIP_BED%bam}${t}.bedpe  > ${MULTIP_BED%bam}${t}.bedpe.ct " >> $parafly_file
done
