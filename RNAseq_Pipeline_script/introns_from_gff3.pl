...
if (defined($opt_d)) {
	...
	if (0) {
	    ...
	    $FP = new IO::File("| /home/wangj2/bin/get-genome -D $opt_D -d $opt_d > get-genome.out");
	} else {
	    $FP = new IO::File("| /home/wangj2/bin/get-genome -d $opt_d > get-genome.out");
	}
...
sub print_exons {
	...
	for ($i = 0; $i < $nexons - 1; $i++) {
	    ...
	    if (!defined($opt_d)) {
		#printf ">%s.%s.intron%d/%d %s:%u..%u\t%s\n",$gene_name,$transcript_id,$i+1,$nexons-1,$chr,$ {$ends}[$i]+1,$ {$starts}[$i]-1,$strand;
		$s=${$ends}[$i]+1;
		$e=${$starts}[$i]-1;
		print "$chr\tQiang-Cufflnk\tintron\t$s\t$e\t.\t$strand\t.\tID=$gene_name;Name=$transcript_id\n";
	    } else {
		$query = sprintf("%s:%u..%u",$chr,$ends[$i]+1,$ends[$i]+2);
